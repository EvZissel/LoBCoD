...
%   Usage:
% 
%   [cleanI,objective,avgpsnr,sparsity,totTime,alpha,D] = LoBCoD_online(params)
% 
%   Main parameters:
%
...
params_pursuit = [];
params_pursuit.Ytrain = I_test;
params_pursuit.lambda = lambda;
params_pursuit.MAXITER = 50; 
params_pursuit.D = D;
params_pursuit.Train_on = false(1);
...
for outerIter = 1 : MAXITER
    ...
    for ii=1:N
        ...
        tElapsed = toc(tStart);
        sumsec = sumsec+tElapsed;
        if (mod(kk,eval_step)==0)
            params_pursuit.D = D;
            [~,obj_test,~,~,~,~,~] = LoBCoD(params_pursuit);
            objective_Test(jf) = obj_test(end);
            time_Test(jf) = sumsec;             
            k_vec(jf) = kk;
            jf = jf+1;
            fprintf('k = %d, Obj test = %.3d \n',kk,objective_Test(jf-1));
        end
        tStart = tic;
        if ((mod(kk,eval_step)==0) && (kk>0))
             eta2 = eta2/(1+(eval_step/kk));
        end
        kk = kk + 1;
